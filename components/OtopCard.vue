    ...
    <div class="card">
        <div class="card-header d-flex flex-column flex-lg-row bg-white">
            <div class="d-flex align-items-center">
                ...
                <h5 class="card-title mb-0">{{ otop.name }}</h5>
            </div>

            <div class="d-flex align-items-center ml-auto">
                <router-link :to="`/otops/${otop.id}/edit`" class="btn btn-sm btn-default">
            ...
            <div class="row">
                <div class="col-lg-5">
                    ...
                    <div class="position-relative d-flex mb-2 bg-light rounded embed-responsive embed-responsive-1by1">
                        <template v-if="otop.media_1">
                            <img :src="otop.media_1.url" class="rounded img-fluid w-100" />
                            <a @click.prevent="deletingMedia(otop.media_1.id)" href="#" class="position-absolute" style="top: 6px; right: 6px;">
                    ...
                    <div class="row no-gutters">
                        <div class="col px-1">
                            ...
                            <div class="position-relative d-flex bg-light rounded embed-responsive embed-responsive-1by1">
                                <template v-if="otop.media_2">
                                    <img :src="otop.media_2.url" class="rounded img-fluid w-100" />
                                    <a @click.prevent="deletingMedia(otop.media_2.id)" href="#" class="position-absolute" style="top: 6px; right: 6px;">
                                        <span class="material-icons-outlined">close</span>
                                    </a>
                                </template>
                                <template v-else>
                        ...
                        <div class="col px-1">
                            <!-- file 3 -->
                            <div class="position-relative d-flex bg-light rounded embed-responsive embed-responsive-1by1">
                                <template v-if="otop.media_3">
                                    <img :src="otop.media_3.url" class="rounded img-fluid w-100" />
                                    <a @click.prevent="deletingMedia(otop.media_3.id)" href="#" class="position-absolute" style="top: 6px; right: 6px;">
                                        <span class="material-icons-outlined">close</span>
                                    </a>
                                </template>
                                <template v-else>
                        ...
                        <div class="col px-1">
                            <!-- file 4 -->
                            <div class="position-relative d-flex bg-light rounded embed-responsive embed-responsive-1by1">
                                <template v-if="otop.media_4">
                                    <img :src="otop.media_4.url" class="rounded img-fluid w-100" />
                                    <a @click.prevent="deletingMedia(otop.media_4.id)" href="#" class="position-absolute" style="top: 6px; right: 6px;">
                                        <span class="material-icons-outlined">close</span>
                                    </a>
                                </template>
                                <template v-else>
                ...
                <div class="col-lg-7">
                    <table class="table">
                        <tr class="d-flex flex-column d-lg-table-row">
                            ...
                            <td class="border-0">{{ otop.name }}</td>
                        </tr>

                        <tr class="d-flex flex-column d-lg-table-row">
                            <td class="border-0 text-nowrap font-weight-bold">ร้านค้า</td>
                            <td class="border-0">{{ otop.shop_name }}</td>
                        </tr>

                        <tr class="d-flex flex-column d-lg-table-row">
                            <td class="border-0 text-nowrap font-weight-bold">จำนวน</td>
                            <td class="border-0">{{ otop.amount | number }}</td>
                        </tr>

                        <tr class="d-flex flex-column d-lg-table-row">
                            <td class="border-0 text-nowrap font-weight-bold">ราคา</td>
                            <td class="border-0">{{ otop.price | number }} บาท</td>
                        </tr>

                        <tr class="d-flex flex-column d-lg-table-row">
                            <td class="border-0 text-nowrap font-weight-bold">คำอธิบาย</td>
                            <td class="border-0">{{ otop.description }}</td>
                        </tr>
                    </table>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        deleting () {
            this.delete(this.otop.id).then(
                () => this.$router.push('/otops')
            )
        },
        deletingMedia (id) {
            this.deleteMedia(id).then(() => this.find(this.otop.id))
        },
