    ...
    <div class="card">
        <div class="card-header d-flex flex-column flex-lg-row bg-white">
            <div class="d-flex align-items-center">
                ...
                <h5 class="card-title mb-0">{{ transaction.name }}</h5>
            </div>

            <div class="d-flex align-items-center ml-auto">
                <router-link :to="`/expenses/${transaction.id}/edit`" class="btn btn-sm btn-default">
            ...
            <div class="row">
                <div class="col-lg-5">
                    ...
                    <div class="position-relative d-flex mb-2 bg-light rounded embed-responsive embed-responsive-1by1">
                        <template v-if="transaction.media_1">
                            <img :src="transaction.media_1.url" class="rounded img-fluid w-100" />
                            <a @click.prevent="deletingMedia(transaction.media_1.id)" href="#" class="position-absolute" style="top: 6px; right: 6px;">
                    ...
                    <div class="row no-gutters">
                        <div class="col px-1">
                            ...
                            <div class="position-relative d-flex bg-light rounded embed-responsive embed-responsive-1by1">
                                <template v-if="transaction.media_2">
                                    <img :src="transaction.media_2.url" class="rounded img-fluid w-100" />
                                    <a @click.prevent="deletingMedia(transaction.media_2.id)" href="#" class="position-absolute" style="top: 6px; right: 6px;">
                                        <span class="material-icons-outlined">close</span>
                                    </a>
                                </template>
                                <template v-else>
                        ...
                        <div class="col px-1">
                            <!-- file 3 -->
                            <div class="position-relative d-flex bg-light rounded embed-responsive embed-responsive-1by1">
                                <template v-if="transaction.media_3">
                                    <img :src="transaction.media_3.url" class="rounded img-fluid w-100" />
                                    <a @click.prevent="deletingMedia(transaction.media_3.id)" href="#" class="position-absolute" style="top: 6px; right: 6px;">
                                        <span class="material-icons-outlined">close</span>
                                    </a>
                                </template>
                                <template v-else>
                        ...
                        <div class="col px-1">
                            <!-- file 4 -->
                            <div class="position-relative d-flex bg-light rounded embed-responsive embed-responsive-1by1">
                                <template v-if="transaction.media_4">
                                    <img :src="transaction.media_4.url" class="rounded img-fluid w-100" />
                                    <a @click.prevent="deletingMedia(transaction.media_4.id)" href="#" class="position-absolute" style="top: 6px; right: 6px;">
                                        <span class="material-icons-outlined">close</span>
                                    </a>
                                </template>
                                <template v-else>
                ...
                <div class="col-lg-7">
                    <table class="table">
                        <tr class="d-flex flex-column d-lg-table-row">
                            ...
                            <td class="border-0">{{ transaction.name }}</td>
                        </tr>

                        <tr class="d-flex flex-column d-lg-table-row">
                            <td class="border-0 text-nowrap font-weight-bold">จำนวน</td>
                            <td class="border-0">{{ transaction.amount | number }} {{ transaction.unit }}</td>
                        </tr>

                        <tr class="d-flex flex-column d-lg-table-row">
                            <td class="border-0 text-nowrap font-weight-bold">ราคาต่อหน่วย</td>
                            <td class="border-0">{{ transaction.unit_price | number }} บาท</td>
                        </tr>

                        <tr class="d-flex flex-column d-lg-table-row">
                            <td class="border-0 text-nowrap font-weight-bold">วันที่</td>
                            <td class="border-0">{{ transaction.date | date }}</td>
                        </tr>

                        <tr class="d-flex flex-column d-lg-table-row">
                            <td class="border-0 text-nowrap font-weight-bold">ราคา</td>
                            <td class="border-0">{{ Math.abs(transaction.total) | number }} บาท</td>
                        </tr>
...
export default {
    ...
    methods: {
        ...
        deleting () {
            this.delete(this.transaction.id).then(
                () => this.$router.push('/expenses')
            )
        },
        deletingMedia (id) {
            this.deleteMedia(id).then(() => this.find(this.transaction.id))
        },
        uploadingMedia (e, index) {
            ...
            }).then(() => this.find(this.transaction.id))
        }
    }
}
</script>
