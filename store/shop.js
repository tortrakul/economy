...
const actions = {
  async all ({ commit, state }, meta) {
    ...
    await this.$axios.$get('/shops', { params }).then(
      ({ data, meta }) => {
        commit('all', data)
  ...
  async find ({ commit }, id) {
    await this.$axios.$get(`/shops/${id}`).then(
  ...
  async create (ctx, payload) {
    ...
    await this.$axios.$post('/shops', form, config)
  },
  async update (ctx, payload) {
    await this.$axios.$patch(`/shops/${payload.id}`, payload)
  },
  async delete (ctx, id) {
    await this.$axios.$delete(`/shops/${id}`)
  }
}
